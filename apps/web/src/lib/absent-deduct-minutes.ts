 ...
 * ★ 2026-08-08: 回 { minutes, deductedLunch } 令 exceptions route 共用邏輯
 */
export function computeAbsentDeductMinutes(
  ...
  lunchMinutes: number,
): { minutes: number; deductedLunch: boolean } {
  ...
  return {
    minutes: Math.max(0, rawMinutes - (dayDeducts ? lunchMinutes : 0)),
    deductedLunch: dayDeducts,
  }
}
