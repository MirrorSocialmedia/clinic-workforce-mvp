...
function getShiftCode(shift: Shift): string {
  ...
  const tcEmployeeGroups = useMemo(() => {
    ...
    // ★ cwm-attexempt-20260914：免考勤員工（會計）唔出現喺排班
    const pool = employees.filter(e => e.status !== 'RESIGNED' && e.attendanceExempt !== true)
  ...
  const trueLocalEmployeeIds = useMemo(() => {
    ...
    return new Set(
      employees
        ...
        .filter(emp => emp.status !== 'RESIGNED' && emp.attendanceExempt !== true) // ★ cwm-attexempt-20260914：免考勤唔排更
  ...
  const monthlyWorkHours = useMemo(() => {
    ...
    const hoursEmps = employees.filter(e =>
      e.status !== 'RESIGNED' && e.attendanceExempt !== true && // ★ cwm-attexempt-20260914：免考勤唔排更
  ...
  const allEmployees = useMemo(() => {
    // ★ cwm-resignflow-20260911 D：白名單改黑名單
    // ★ cwm-attexempt-20260914：免考勤員工（會計）唔出現喺排班
    return employees.filter(emp => emp.status !== 'RESIGNED' && emp.attendanceExempt !== true)
  }, [employees])
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 9, tableLayout: 'fixed' }}>
                  <thead>
                    <tr style={{ background: '#eff6ff', color: '#1d4ed8' }}>
                      ...
                      {/* ★ 2026-08-22 §6.3：上月剩（上月快照）+ 剩餘（截至當月）—— 兩欄新加 */}
                      {/* ★ 2026-09-01 cwm-lmr：休息日截到上月底，年假欄係即時值（混合口徑，拍板②） */}
                      <th style={{ width: 44, padding: '5px', background: '#dbeafe' }}
                          title="休息日截至上月底；年假為即時值">上月剩</th>
                      {/* ★ 2026-09-05 cwm-lvsum：剷 R／PL 兩欄（留 R+PL；tableLayout fixed 下重分寬度）；
                           剩餘加「截至 MM」標籤（API 已改截至當月，一眼知道跟住月份變） */}
                      <th style={{ width: 52, padding: '5px', background: '#93c5fd', color: '#1e3a8a' }}>R+PL</th>
                      <th style={{ width: 82, padding: '5px', background: '#dbeafe' }}>
                        剩餘 <span style={{ fontWeight: 400, fontSize: 8 }}>截至 {ovMonth.slice(5)}</span>
                      </th>
                    ...
                    {summaryRows.map(r => (
                      <tr key={r.employeeId} style={{ borderBottom: '0.5px solid #e5e7eb' }}>
                        ...
                        {/* ★ 2026-08-22 §6.3：上月剩 —— 上月 snapshot（REST_DAY）；
                             ...
                             ★ 2026-09-05 cwm-lvsum：剷走 ~ 視覺記號（title tooltip 保留 snapshot／動態之分） */}
                        ...
                        {/* ★ 2026-09-05 cwm-lvsum：剷 R／PL 兩欄 + 「少過配額」警告同紅色
                             （API restDays／plDays／restQuota 照回 —— 拍板① 只剷前端） */}
                        <td style={{ padding: '5px', textAlign: 'center', fontWeight: 600 }}>
                          {r.total}
                        </td>
                        {/* ★ 2026-09-05 cwm-lvsum：剩餘 = REST_DAY 結餘「截至當月月底」（API restDayBalanceAsOf asOf=本月尾）。
                             唔係「上月剩 − R − PL」推導（後者未計本月發放），所以加 title 提示。 */}
                        <td style={{ padding: '5px', textAlign: 'center', fontWeight: 600 }}
                            title="截至當月 = REST_DAY 結餘截至當月月底">
                        ...
                        <td style={{ padding: '4px 5px', borderLeft: '2px solid #60a5fa', wordBreak: 'break-word' }}>
                          ...
                          {/* ★ cwm-annualdisp-b-20260911 方案乙：餘額大字行頭，其餘退做細字。
                               ★★★「餘」= LeaveBalance.remaining —— 權威值，
                               ❌ 唔好由「累積 − 已放」推（Celia 會變 8−8=0，−1.2 訊息消失）。
                               第 2 行嘅算式係【說明】，大字係【權威】；rounding 令兩者差 0.1 時以大字為準。
                               ★「已放」= max(假期單, 餘額反推) + 試用期 gate（cwm-annualused 方案 D）→ tooltip 保留。 */}

                          {/* 第 1 行：餘額大字 */}
                          <div style={{ lineHeight: 1.4, display: 'flex', alignItems: 'baseline', flexWrap: 'wrap', gap: 4 }}>
                            <span style={{ fontSize: 16, fontWeight: 600,
                                           color: r.balanceRemaining < 0 ? '#dc2626' : '#059669' }}>
                              {r.balanceRemaining}
                            </span>
                            <span style={{ fontSize: 10, color: '#64748b' }}>天可放</span>
                            {r.balanceRemaining < 0 && (
                              <span style={{ background: '#fee2e2', color: '#b91c1c', borderRadius: 3,
                                             padding: '1px 6px', fontSize: 9, fontWeight: 600 }}>已預支</span>
                            )}
                          ...
                          {/* 第 2 行：算式 ＋ 配額 ＋ 狀態標籤
                               ⚠️ 冇結轉時顯示 available 唔係 accruedThisYear —— 否則同大字差 0.1；
                               「本年累積」= available − carryOver（rounding 後加得返） */}
                          <div style={{ color: '#64748b', fontSize: 9, lineHeight: 1.5 }}>
                            <span style={{ fontFamily: 'monospace' }}>
                              {r.carryOver > 0.5
                                ? `結轉 ${r.carryOver.toFixed(1)}　＋　累積 ${r1(r.available - r.carryOver)}　−　`
                                : `累積 ${r.available}　−　`}
                              <span title={
                                `已放 = max(系統假期單, 由餘額反推)\n` +
                                `　系統假期單：${r.takenDays} 日\n` +
                            ...
                            {'　·　配額 '}{r.entitled}{' 天/年'}
                            {r.underOneYear && <span style={{ color: '#b45309' }}>{'　'}⚠️ 未滿一年</span>}
                            {r.inProbation && <span style={{ color: '#b45309' }}>{'　'}· 試用期</span>}
                          </div>

                          {/* 第 3 行：服務年度區間 ＋ 假期單（合併，原本第 3、4 行） */}
                          <div style={{ color: '#94a3b8', fontSize: 9, lineHeight: 1.5 }}>
                            {r.syStart}～{r.syEnd}
                            {r.takenDates && `　·　假期單 ${r.takenDates}`}
                          </div>
                        </td>
                      </tr>
                    ))}
