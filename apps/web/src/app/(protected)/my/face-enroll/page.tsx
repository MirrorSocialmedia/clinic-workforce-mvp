  ...
  const startCapture = useCallback(async () => {
    ...
    try {
      ...
      for (const h of hints) {
        setHint(h)
        const b = await captureQualified(videoRef.current!, 5000)
        if (!b) {
          setError('光線不足或未偵測到臉，請重試')
      ...
      {step === 'code' && (
        <div>
          ...
          <button className="w-full mt-4 py-3 bg-blue-600 text-white rounded-lg disabled:opacity-50"
            disabled={code.length !== 6} onClick={() => setStep('consent')}>
            下一步
          </button>
        </div>
      )}
      ...
      {step === 'capture' && (
        <div className="text-center">
          <div className="relative mb-4">
            ...
            {hint && <div className="mt-3 text-lg font-bold">{hint}</div>}
          </div>
          {error && <div className="text-red-600 text-sm mb-3">{error}</div>}
          {!error && (
            <button className="py-2 px-6 bg-blue-600 text-white rounded-lg" onClick={startCapture}>
              開始拍攝
            </button>
          )}
        </div>
