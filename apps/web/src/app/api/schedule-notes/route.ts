...
export async function PUT(req: NextRequest) {
  ...
  const { companyId, date, text } = await req.json()
  ...
  if (!clean) {
    await prisma.scheduleNote.deleteMany({ where: { companyId, date } })
  ...
  const note = await prisma.scheduleNote.upsert({
    ...
    create: { companyId, date, text: clean, createdBy: session.userId },
  })
